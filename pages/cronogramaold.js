...
export default function Cronograma() {
  ...
  function openNoteModal(dayKey, shiftKey, slotKey, currentVal) {
    ...
    // marca como lida para o usuário atual ao abrir
    ...
    <AuthGate requireRole="cronograma">
      <div className={`min-h-screen bg-gradient-to-b from-slate-950 via-emerald-900 to-emerald-800 text-white relative overflow-hidden ${hiContrast ? "contrast-high" : ""}`}>
        ...
        <main className="max-w-7xl mx-auto px-4 py-5 space-y-4">
          {/* Legenda */}
          <div className="rounded-xl border border-white/15 bg-white/10 p-2 shadow-xl overflow-x-auto">
          ...
          {/* Filtros + navegação */}
          <div className="rounded-xl border border-white/15 bg-white/10 p-2.5 shadow-xl flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-3 flex-wrap">
              {/* filtro usuário */}
              <div className="flex items-center gap-2">
                <label className="text-[11px] text-gray-200">Filtrar por usuário:</label>
                <select
                  className="dark-select bg-black/40 border border-white/10 rounded px-3 py-2 text-sm cursor-pointer hover:bg-white/5"
                  value={filterUid}
          ...
          {loadingData && (
            <div className="rounded-xl border border-white/15 bg-white/10 p-3 text-sm text-gray-200 animate-pulse">
              Carregando escala…
            </div>
          )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-2 md:gap-3 min-w-[980px]">
              {days.map((d, idx) => {
                ...
                return (
                  <div
                    key={key}
                    className={`rounded-xl border border-white/15 bg-white/10 shadow-lg flex flex-col transition
                      ${isToday ? "ring-2 ring-emerald-500 border-emerald-500 " : ""}${filterUid ? (visible ? "" : " opacity-35") : ""} 
                      animate-[fadeIn_.28s_ease-out]`}
                  >
                    {/* cabeçalho do dia */}
                    <div className="px-2 pt-2 pb-1 sticky top-0 z-10 bg-white/10 backdrop-blur border-b border-white/15">
                                ...
                                <div className="flex items-center gap-1">
                                  {canEdit && (
                                    <button
                                      ...
                                      className="relative text-[10px] px-2 py-0.5 rounded cursor-pointer transition bg-white/10 hover:bg-white/20 text-white/90 ring-1 ring-white/15"
                                      title={hasNote ? "Editar anotação (existe anotação)" : "Adicionar anotação"}
                                    >
                                      Nota
                                      {unread && (
                                        <span
                                          className="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-red-500 ring-1 ring-white/80"
                                          title="Nota não lida"
                              ...
                              <div className="flex items-center gap-2">
                                {assignedUser ? (
                                  ...
                                  <button
                                    ...
                                    className="flex-1 bg-black/40 border border-white/15 rounded px-3 py-0.5 text-sm text-left hover:bg-white/5 cursor-pointer"
